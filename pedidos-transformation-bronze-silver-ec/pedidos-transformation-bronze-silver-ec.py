...
AmazonS3 = spark.read.options(delimiter="|", header=True).csv("s3://pedidos-bronze/ecommerce")
...
AmazonS3 = AmazonS3\
    ...
    .withColumn("frete", col("frete").cast(DecimalType(10, 2)))\
    .withColumn("tipo_de_entrega", col("tipo_de_entrega").cast(StringType()))\
    .withColumn("entrega_estimada", col("entrega_estimada").cast(TimestampType()))\
...
AmazonS3 = AmazonS3.withColumn("tipo_de_entrega", lower(AmazonS3["tipo_de_entrega"])) \
             .withColumn("tipo_de_entrega", regexp_replace("tipo_de_entrega", "retirar naloja", "retirar na loja"))
...
tabela = "pedidos_ecommerce"
output_path = "s3://pedidos-silver/ecommerce/"
